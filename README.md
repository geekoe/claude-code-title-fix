...
### 问题一：标题太短且是英文（fix-title-prompt.sh）
...
只要切换到其他 tab，Claude Code 所在的 tab 就会立刻出现未读标记。这是 Claude Code 的已知问题（[#7121](https://github.com/anthropics/claude-code/issues/7121)、[#17060](https://github.com/anthropics/claude-code/issues/17060)），目前尚无可靠的本地修复方案。临时解决办法是在终端设置中关闭 bell 通知（如 iTerm2: Profiles → Terminal → 取消 Audible/Visual Bell）。
...
# 修复标题不更新
curl -fsSL https://raw.githubusercontent.com/geekoe/claude-code-title-fix/main/fix-title-update.sh | bash
```
...
bash claude-code-title-fix/fix-title-prompt.sh
bash claude-code-title-fix/fix-title-update.sh
```
...
### Issue 1: Title too short (fix-title-prompt.sh)
...
### Issue 2: Title not updating (fix-title-update.sh)
...
### Known Issue: Tab keeps showing unread badge

Whenever you switch away from the Claude Code tab, it immediately shows an unread indicator. This is a known Claude Code issue ([#7121](https://github.com/anthropics/claude-code/issues/7121), [#17060](https://github.com/anthropics/claude-code/issues/17060)) with no reliable local fix yet. As a workaround, disable bell notifications in your terminal (e.g., iTerm2: Profiles → Terminal → uncheck Audible/Visual Bell).
